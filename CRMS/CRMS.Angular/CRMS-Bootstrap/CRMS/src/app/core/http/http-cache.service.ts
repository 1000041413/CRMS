...
export class HttpCacheService {
   ...
   * @param {HttpHeaders} data The received data.
   * @param {Date=} lastUpdated The cache last update, current date is used if not specified.
   */
  setCacheData(url: string, params: any, data: HttpHeaders, lastUpdated?: Date) {
   ...
   */
  getCacheData(url: string, params?: any): HttpHeaders {
    const cacheKey = this.getCacheKey(url, params);
  ...
  clearCache(url: string, params?: any): void {
    const cacheKey = this.getCacheKey(url, params);
    this.cachedData[cacheKey] = undefined as any;
  ...
  setPersistence(persistence?: 'local'|'session') {
    this.cleanCache();
    this.storage = persistence === 'local' || persistence === 'session' ? window[(persistence + 'Storage') as any] : null;
    this.loadCacheData();
  }
